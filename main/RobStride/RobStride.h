...
// Command types
#define RobStride_Set_mode          'j'        // Set control mode
#define RobStride_Set_parameter     'p'        // Set parameter
...
static const uint16_t RobStride_Index_List[] = {
...
typedef struct
{
	RobStride_data_read_write_one run_mode;        // 0: torque mode, 1: position mode, 2: speed mode,
										 // 3: current mode, 4: zero mode (uint8, 1 byte)

	RobStride_data_read_write_one iq_ref;          // Current mode Iq reference     (float, 4 bytes, -23~23 A)
	RobStride_data_read_write_one spd_ref;         // Speed mode target speed       (float, 4 bytes, -30~30 rad/s)
	RobStride_data_read_write_one imit_torque;     // Torque limit                  (float, 4 bytes, 0~12 Nm)
	RobStride_data_read_write_one cur_kp;          // Current loop Kp               (float, default 0.125)
	RobStride_data_read_write_one cur_ki;          // Current loop Ki               (float, default 0.0158)
	RobStride_data_read_write_one cur_filt_gain;   // Current loop filter gain      (float, default 0.1)
	RobStride_data_read_write_one loc_ref;         // Position mode reference       (float, rad)
	RobStride_data_read_write_one limit_spd;       // Speed limit in position mode  (float, 0~30 rad/s)
	RobStride_data_read_write_one limit_cur;       // Current limit                 (float, 0~23 A)

	// Read-only parameters
	RobStride_data_read_write_one mechPos;         // Mechanical absolute angle     (float, rad)
	RobStride_data_read_write_one iqf;             // Filtered iq                   (float, -23~23 A)
	RobStride_data_read_write_one mechVel;         // Mechanical angular velocity   (float, -30~30 rad/s)
	RobStride_data_read_write_one VBUS;            // Bus voltage                   (float, V)
	RobStride_data_read_write_one rotation;        // Number of turns               (int16, 2 bytes)
} RobStride_data_read_write;

void RobStride_data_read_write_init(RobStride_data_read_write *this, const uint16_t *index_list /* = Index_List */);
...
} RobStride_Motor_Pos_RobStride_Info;
...
typedef struct {
    ...
    RobStride_Motor_Set Motor_Set_All;      // Motor set values
    uint8_t error_code;

    bool MIT_Mode;                // MIT mode flag
    RobStride_MIT_TYPE MIT_Type;            // MIT control type

    float output;
    int Can_Motor;
    RobStride_Motor_Pos_RobStride_Info Pos_Info;     // Feedback values
    RobStride_data_read_write drw;                   // Parameter read/write
...
void RobStride_Set_MIT_Type(RobStride *robstride, RobStride_MIT_TYPE MIT_Type);
...
RobStride_MIT_TYPE RobStride_get_MIT_Type(RobStride *robstride);
